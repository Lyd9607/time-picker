...
(function($) {
  $.fn.mdater = function(config) {
    ...
    var mdater = {
      ...
      renderHTML: function() {
        var $html = $('<div class="md_mask"></div><div class="md_panel"><div class="md_bar"> <span class="md_cancel">取消</span><span class="md_ok">完成</span></div><div class="md_head"><div class="md_selectarea"><span class="md_prev change_year"><</span><span class="md_headtext yeartag"></span><span class="md_next change_year">></span></div><div class="md_selectarea"><span class="md_prev change_month"><</span><span class="md_headtext monthtag">月</span> <span class="md_next change_month" >></span></div></div><div class="md_body"><ul class="md_weekarea"><li>日</li><li>一</li><li>二</li><li>三</li><li>四</li><li>五</li><li>六</li></ul><ul class="md_datearea in"></ul></div></div>');
        if ($('.md_mask').length == 0) { $(document.body).append($html); }
        return $html;
      },
      _showPanel: function(container) {
        this.datePicker.find('.drop-icon').css({transform: 'rotate(135deg)'});
        this.refreshView();
        $('.md_panel, .md_mask').addClass('show');
      },
      _hidePanel: function() {
        this.datePicker.find('.drop-icon').css({transform: 'rotate(45deg)'});
      ...
      refreshView: function() {
        this.datePicker.find('.drop-icon').css({transform: 'rotate(135deg)'});
        var initVal = this.datePicker.find('.drop-text').html();
        var date = null;
        if (initVal) {
      ...
      datePicker: null, //暂存当前指向input
      initListeners: function() {
        var _this = this;
        datePicker.on('click', function() {
          _this.datePicker = $(this); //暂存当前指向input
          if ($('.md_mask').length) {
            _this._hidePanel();
          } else {
            _this.renderHTML();
      ...
      saveValueToInput: function() {
        ...
        _this.datePicker.find('.drop-text').html(_this.value.year + '-' + monthValue + '-' + dateValue);
        _this._hidePanel();
      },
...
if($('.date').length){
  (function initDate(){
    ...
    $('.date').find('.drop-text').html(dateString);
  })();
}

$('.date .drop-item').mdater();